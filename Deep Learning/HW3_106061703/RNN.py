...
model.add(SimpleRNN(75,input_shape=(X.shape[1],X.shape[2]),batch_size=batch,return_sequences=True,stateful=True))
model.add(Dropout(0.2))
#model.add(LSTM(100))
model.add(SimpleRNN(100,batch_size=batch,stateful=True))
model.add(Dropout(0.2))
...
#fit model
train_history=model.fit(X,y,validation_data=(X_valid,y_valid),epochs=30,batch_size=batch,verbose=2)
